...
#if defined(OLED_I2C)
/* OLED I2C Callback */
writeCallback i2cWriteCb = NULL;
#elif defined(OLED_SPI)
/* OLED SPI Callback */
writeCallback spiWriteCb = NULL;
#endif
...
static bool OLED_WriteCmd(uint8_t cmd) {
	return i2cWriteCb(OLED_I2C_ADDR << 1, &cmd, 1);
}

static bool OLED_WriteData(const uint8_t data[], uint32_t len) {
	return i2cWriteCb(OLED_I2C_ADDR << 1, data, len);
}

bool OLED_Init(writeCallback cb) {
	if (cb == NULL)
		return false;
	i2cWriteCb = cb;
	m_width = OLEDWIDTH;
	...
	while (i < sizeof(SSD1306_regValues) / sizeof(SSD1306_regValues[0])) {
		...
		success &= OLED_WriteCmd(r);
		for (uint8_t d = 0; d < len; d++) {
			uint8_t x = SSD1306_regValues[i++];
			success &= OLED_WriteCmd(x);
		}
	}
#elif defined(SH1106)
	uint8_t i = 0;
