...
#if defined(OLED_I2C)
typedef bool (*writeCallback)(uint8_t address, const uint8_t data[], uint32_t len);
#else if defined(OLED_SPI)
typedef bool (*writeCallback)(const uint8_t data[], uint32_t len);
#endif
